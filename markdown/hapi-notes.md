...
- There are two places to put your options: 
    - **View Manager Options**: when options are put at the top level, they will affect all engines, so this is a good place to put things like *context*
...
- **isCached** (optional, BUT NEEDED FOR SANITY)
    ...
    - in production, it should be set to true in order to increase performance, so it's common to see it set as a boolean that pulls in environment variables or something
